...
exports.methods = {
    ...
    async buildVisualScene() {
        ...
        if (oldLayout) {
            /** 判断现有布局是否包含完整的核心区域。 */
            const hasCompleteLayout = ["WordBoard", "LetterWheel", "EndCard"].every(
                (nodeName) => Boolean(oldLayout.getChildByName(nodeName)),
            );
            /** 现有布局记录的生成版本。 */
            const oldBuildVersion = oldLayout.__editorExtras__
                && oldLayout.__editorExtras__.visualBuildVersion;
            if (hasCompleteLayout && oldBuildVersion === VISUAL_BUILD_VERSION) {
                return {
                    skipped: true,
                    reason: "PlayableLayout already exists",
                };
            }
        ...
        /** 单词棋盘根节点。 */
        const board = createNode("WordBoard", layoutRoot, 500, 440, 0, 250);
        board.setScale(1.15, 1.15, 1);
        /** 需要显示的全部单词。 */
        ...
        words.forEach((word, rowIndex) => {
            ...
            /** 当前行橙色发光背景。 */
            const glowNode = createNode("ActiveGlow", rowNode, rowWidth + 22, 68);
            createSprite(
                "GlowOuter",
                glowNode,
                roundedFrame,
        ...
        /** 字盘根节点。 */
        const wheel = createNode("LetterWheel", layoutRoot, 470, 470, 0, -250);
        createSprite(
            "WheelOuter",
            wheel,
            circleFrame,
        ...
        for (let letterIndex = 0; letterIndex < 7; letterIndex += 1) {
            ...
            /** 字母标签。 */
            const letterLabel = letterNode.getComponent(Label);
        ...
        /** 圆盘右下角下载箭头。 */
        const downloadButton = createNode("DownloadButton", layoutRoot, 90, 90, 180, -455);
        /** 下载图标节点。 */
        ...
        /** 页面右下角安装面板。 */
        const installPanel = createNode("InstallPanel", layoutRoot, 240, 70, 220, -590);
        createSprite(
            "DownloadGameIcon",
            installPanel,
            gameIconFrame,
        ...
        /** 安装按钮节点。 */
        const installButton = createNode("InstallButton", installPanel, 170, 64, 30, 0);
        createSprite(
            "InstallBackground",
            installButton,
            roundedFrame,
        ...
        endCardIcon.setScale(1.65, 1.65, 1);
        /** 结束页品牌面板。 */
        const brandPanel = createNode("BrandPanel", endCard, 500, 86, 0, 80);
        createSprite(
            "BrandGlow",
            brandPanel,
            roundedFrame,
        ...
        brandPanel.setScale(1.12, 1.12, 1);
        /** 结束页主按钮。 */
